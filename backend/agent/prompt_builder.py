...
- **`add_title` 도 `add_auto_subtitle` 과 마찬가지로 기본 `burn=false` 로 호출한다 —
  절대 `burn=true` 를 임의로 넣지 마라.** burn=true 로 구우면 그 즉시 픽셀이 되어 자막
  cue 레이어와 완전히 분리된 별도 갈래(branch)가 생긴다 — 실제 사고: add_title 을
  burn=true 로 구운 파일과, 그 이전 파일에서 바로 이어 add_auto_subtitle 을 부른
  파일이 서로 다른 산출물로 갈라져서, 최종적으로 자막 쪽 갈래가 쓰이며 제목이 통째로
  빠졌다. `burn=true` 는 사용자가 "제목 지금 바로 구워줘"처럼 명시했을 때만.
- **제목과 발화 자막은 반드시 같은 큐 문서에 쌓이도록 이어라.** 둘 다 burn=false 면
  파일을 그대로 복사만 하고 큐 문서에 기록하는 방식이라, `add_title` 의 `output_path`
  를 다음 `add_auto_subtitle` 의 `video_path` 로 정확히 넘겨야 같은 큐 문서에 title
  큐 + 발화 큐가 함께 쌓인다. 둘이 서로 다른 입력 파일에서 각자 출력을 내면(둘 다
  같은 이전 step 산출물을 입력으로 쓰면서 서로 못 이어지면) 한쪽 큐가 완전히
  누락된 채로 최종 결과물이 나간다.
